    ...
    public function getWidgetDeclaration($type, $params = [], $asIs = true)
    {
        ...
        foreach ($params as $k => $value) {
            ...
            if(is_array($params[$k]) || !base64_decode($params[$k], true)) {
                if(in_array($k, $field_pattern) || preg_match("/^tabs(.*)/", $k) || preg_match("/^link(.*)/", $k) || preg_match("/^href(.*)/", $k) || preg_match("/^html_(.*)/", $k) || preg_match("/^content_(.*)/", $k) || (preg_match("/^header_(.*)/", $k) && in_array($type, $widget_types))) {
