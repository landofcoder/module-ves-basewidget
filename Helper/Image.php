	...
	public function resizeImage($image, $width = 100, $height = 100, $qualtity = 100, $keep_ratio = true){
        ...
        if (!file_exists($_imageResized)&&file_exists($_imageUrl)){
            ...
            if($keep_ratio) {
                ...
                $org_width = $imageObj->getOriginalWidth();
                $org_height = $imageObj->getOriginalHeight();
                $org_height = $org_height?(int)$org_height:1;
                $currentRatio = $org_width / $org_height;
                if($height){
                  $targetRatio = $width / $height;
                } else {
                  $targetRatio = $currentRatio+1;
                }
                if ($targetRatio > $currentRatio) {
                  if(!$height){
                    $custom_height = floor(($width*$org_height)/$org_width);
                    $imageObj->resize($width, $custom_height);
                  } else {
