<?php
namespace Ves\BaseWidget\Classes;
/**
...
class ElFinder {
	...
	public function __construct($opts) {
		...
		if (isset($opts['roots']) && is_array($opts['roots'])) {
			...
			foreach ($opts['roots'] as $i => $o) {
				$class = '\Ves\BaseWidget\Classes\Elfinder\ElFinderVolume'.(isset($o['driver']) ? $o['driver'] : '');
				//echo $class."<br/>";
				if (class_exists($class)) {
					$new_opts = array();
					$new_opts['dirMode'] = isset($opts['dirMode'])?$opts['dirMode']:0755;
					$new_opts['fileMode'] = isset($opts['fileMode'])?$opts['fileMode']:0644;
					$new_opts['maxArcFilesSize'] = isset($opts['maxArcFilesSize'])?$opts['maxArcFilesSize']:0;

					$volume = new $class($new_opts);
	...
	public function bind($cmd, $handler) {
		$cmds = $cmd ? array_map('trim', @explode(' ', $cmd)) : [];
