	 		...
	 		$("#widget-modal .btn-submit-shortcode").click( function(event) {
		 		if( $(".wpo-ijwidget.active") ){
	 				...
	 				for( var k in widgetArray ){
	  				...
	  				if(wgcfg.wkey && $("#wpowidget"+wgcfg.wkey).length > 0){
	  					$("#wpowidget"+wgcfg.wkey).val(wgcfg.shortcode);
	  					$("#wpowidget"+wgcfg.wkey+"DIV").html(wgcfg.shortcode);

	  					//Update widgt meta info
	  					updateWidgetShortcodeInfo(wgcfg.shortcode, $(".wpo-ijwidget.active"));
	  					//Load widget meta information
						afterCreateWidget(wgcfg.shortcode, '#widget'+wgcfg.wkey);
						//End load widget meta information
	  				}
	 			}
		...
		function openModalPopup(scol, row, urlajaxcontent, widget_options, requirements, popup_title, save_button_title, cache_key) {
	 	...
	 	function buildWidgetItem( col, widget, content ) {
			...
			if($('#widget'+dw.wkey).length > 0) {
				...
				if(typeof($w.data("wgcfg")) === "undefined"){
					$('#widget'+dw.wkey).data( 'wgcfg', dw );
					$( ".wpo-wcopy", $('#widget'+dw.wkey) ).click( function(){   
						$wpowidget.cloneWidget(  $('#widget'+dw.wkey), col );
					} );

					$( ".wpo-wedit", $('#widget'+dw.wkey) ).click( function(){  
						$wpowidget.editWidget( $('#widget'+dw.wkey) );
					} );

					$( ".wpo-wedit-shortcode", $('#widget'+dw.wkey) ).click( function(){  
						$wpowidget.editShortcode( $('#widget'+dw.wkey) );
					} );

					$( ".wpo-wsetting", $('#widget'+dw.wkey) ).click( function(){  
						$wpowidget.settingWidget( $('#widget'+dw.wkey) );
					} );

					$( ".wpo-wdelete", $('#widget'+dw.wkey) ).click( function(){  
						if( confirm(config.confirmdel) ){
							$('#widget'+dw.wkey).remove();
						}
